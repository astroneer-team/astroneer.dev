                    ...
                    <ul className="px-0.5 last-of-type:mb-0 mb-8">
                      {section.topics.map((topic) => (
                        <li className="my-1.5" key={topic.path}>
                          <Link
                            className={`${inView.includes(topic.path.split('#').pop() ?? '') ? 'text-primary hover:brightness-125 opacity-100' : 'opacity-50'} text-sm py-1 pl-2 text-left hover:opacity-100 transition-all duration-300`}
                            href={topic.path}
                          >
                            {topic.title}
                          </Link>
